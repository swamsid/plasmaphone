...
<div id="ribbon">
	...
	<!-- breadcrumb -->
	<ol class="breadcrumb">
		<li>Home</li><li>Master</li><li>Gudang</li>
	</ol>
	<!-- end breadcrumb -->
	...
	<section id="widget-grid" class="">
		...
		<div class="row">
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="padding: 0px 20px; margin-top: {{ $mt }};">
				<form id="table-form" method="post" action="{{ url('/master/gudang/edit-multiple') }}">
					{!! csrf_field() !!}
					<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
						<thead>			                
							<tr>
								...
								<th data-class="expand"><i class="fa fa-fw fa-building text-muted hidden-md hidden-sm hidden-xs"></i> &nbsp;Kode Gudang</th>
								<th data-hide="phone"><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> &nbsp;Nama Gudang</th>
						...
						<tbody>
							...
							<tr>
								...
								<td class="text-center">
									<input type="checkbox" class="check-me" name="data_check[]" data-id="{{$gudang->g_id}}" value="{{ $gudang->g_id }}"/>
								</td>
								<td>{{ $gudang->g_kode }}</td>
								<td>{{ $gudang->g_nama }}</td>
								<td class="text-center">
									<button class="btn btn-xs btn-warning btn-circle edit" data-toggle="tooltip" data-placement="top" title="Edit Data" data-id="{{ $gudang->g_id }}"><i class="glyphicon glyphicon-edit"></i></button>
									<button class="btn btn-xs btn-danger btn-circle hapus" data-toggle="tooltip" data-placement="top" title="Hapus Data" data-id="{{ $gudang->g_id }}"><i class="glyphicon glyphicon-trash"></i></button>
								</td>
							</tr>
							@endforeach
						</tbody>
...
$(document).ready(function(){
	...
	$("#multiple_delete").click(function(evt){
		...
		if(selected.length == 0){
			...
			if(ask){
				...
				axios.post(baseUrl+'/master/gudang/multiple-delete', {
					data 	: selected,
					_token 	: '{{ csrf_token() }}'
				})
				.then((response) => {
	...
	$(".edit").click(function(evt){
		...
		window.location = baseUrl+'/master/gudang/edit?id='+context.data('id');
	})
