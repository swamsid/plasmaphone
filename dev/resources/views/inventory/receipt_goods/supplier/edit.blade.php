...
@section('ribbon')
	...
	<div id="ribbon">
		...
		<!-- breadcrumb -->
		<ol class="breadcrumb">
			<li>Home</li><li>Inventory</li><li>Penerimaan Barang Dari Supplier</li><li>Edit</li>
		</ol>
		<!-- end breadcrumb -->
			...
			<div class="row">
				<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10 col-md-offset-1" style="padding: 10px 20px; margin-top: 20px; background: #fff;">
					...
					<form id="form-edit" class="form-horizontal" method="post">
						...
						<fieldset>
							<legend>
								Form Edit Penerimaan Barang Dari Supplier
									...
									<div class="form-group">
										...
										<div class="col-xs-8 col-lg-8 inputGroupContainer">
											<select class="form-control" name="supplier" id="supplier">
												<option value="" @if($data[0]->i_id_supplier == 0) selected @endif>---Pilih supplier---</option>
												@foreach($data_supplier as $supplier)
												<option value="{{ $id = $supplier->s_id }}" @if($data[0]->i_id_supplier == $id) selected="selected" @endif>{{ $supplier->s_company }}</option>
												@endforeach
											</select>
										</div>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#id').change(function(evt){
			...
			axios.get(baseUrl+'/inventory/penerimaan/supplier/get-current-receipt/'+context.val())
				.then((response) => {
		...
		function initiate(data){
			if (data.i_imei == null) {
				...
				$("#supplier").val(data.i_id_supplier);
				$('#field_imei').hide();
		...
		$('#form-edit').submit(function(evt){
			...
			if($(this).data('bootstrapValidator').validate().isValid()){
				...
				axios.post(baseUrl+'/inventory/penerimaan/supplier/update', $('#form-edit').serialize())
				.then((response) => {
